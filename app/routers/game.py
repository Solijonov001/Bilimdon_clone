...
from app.schemas import GameCreate,GameResponse,QuestionResponse,GameSelectQuestion,GameUpdate
...
@router.get("/{id}/",response_model=list[GameResponse])
...
@router.post("/ceate/",response_model=GameResponse)
async def create_game(
        ...
        if not game.start_time > game.end_time:
            raise HTTPException(
                status_code=400,
                detail="Start time must be before end found."
            )
        if game.start_time < game.end_time.now(timezone.utc):
            raise HTTPException(
                status_code=400,
...
@router.get("/{id}/quetions",response_model=list[QuestionResponse])
async def game_questions(
        ...
        db_game = db.query(Question).filter(Question.id == id).first()
        ...
        db_questions = db.query(GameQuestion).first(GameQuestion.id == id)

        if db_questions:
            return [db.query(Question).filter(Question.id == question.id.first() for question in db_questions)]
        return []
...
async def select_question(
        ...
        db_game = db.query(Game).filter(Game.id == request.g).first()
