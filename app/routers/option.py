...
@router.get("/",response_model=OptionResponse)
async def get_option(id:int,db:db_dep):
    option = db.query(Option).filter(Option).first()
...
async def ceate_option(
    ...
    existing_correct_option = db.query(Option).filter(
        ...
        Option.is_correct == True).first()

    if existing_correct_option and option.is_correct:
        raise HTTPException(
            status_code=400,
            detail="Question arealdy has a correct option.")
